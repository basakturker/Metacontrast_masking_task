...
def reset_clock_and_send_trigger(value):
    ...
    obj_clock.reset()
def reset_clock_and_send_trigger(value):  # write the trigger values
    obj_clock.reset()
...
for i in range(n_block):
    ...
    for trial in order:
        ...
        soa_dur = int(trial_list_df.loc[trial]['SOA'])
        dotUR.pos = (0.5, 0.5)
        dotUL.pos = (-0.5, 0.5)
        dotDR.pos = (0.5, -0.5)
        dotDL.pos = (-0.5, -0.5)
        ...
        t.setPos([trial_list_df.loc[trial]['position'], 0.0])
        t.contrast=-0.54
        mask_image.setPos(
            [trial_list_df.loc[trial]['position'], 0.0])
        # mask_image.setImage(
...
#        scale_rating.setMarkerPos(random.randint(0,1))
        ...
        for frameN in range(ep_dur):
            ...
            # SOA
            elif target_dur <= frameN < target_dur + soa_dur:
                win.flip()
